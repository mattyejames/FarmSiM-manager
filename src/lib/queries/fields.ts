...
export async function listFields(saveId: string): Promise<Field[]> {
  const db = await getDb();
  return db.select<Field[]>("SELECT * FROM field WHERE save_id = $1 ORDER BY name COLLATE NOCASE", [saveId]);
}
...
export async function createField(saveId: string, input: FieldInput): Promise<string> {
  const db = await getDb();
  const id = uuidv4();
  await db.execute(
    `INSERT INTO field (id, save_id, name, number, size_value, size_unit, soil_type, notes)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
    [id, saveId, input.name, input.number, input.size_value, input.size_unit, input.soil_type, input.notes],
  );
  return id;
}
